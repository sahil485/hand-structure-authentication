...
for i=1:1:5
    for j=1:1:1
        for k=1:1:5
            ...
            for f=freq_min:1e3:freq_max % Loop in steps of 1k
                freq_set(freq_dex) = f;
                freq_dex = freq_dex+1;
            end
            ...
            disp('Filtering...');
            disp(strcat(file_name, ".wav"))

            [x,~] = audioread(strcat(file_name, ".wav")); % read sequence from filename
            ...
            % starting point. There should be no 'tail' at the end of the plot of mic.
                ...
                for v = 1:length(chirp_sig)
                    z_start = lag_a(z(v)); %find the maximum of lag_a
                    ...
                    if z_ending < length(mic_a) && z_start > 0
                        ...
                        usr_cont = input(['#' num2str(cc_counter) ...
                            ' @ Index ' num2str(z(v)) ': Enter 1 to continue, 0 to stop: ']);
                ...
                try
                    ...
                    unprocessed = [unprocessed file_name];
                    continue;
                end
            ...
            for c=1:samples_num
                samples(c,:) = mic_a((c-1)*samples_gap+c*samples_start+1:c*samples_end);
            end
            ...
            % samples)
            for a = 1:samples_num
                for b = 1:2:20 % Up to 20 because there are only 10 chirp repetitions
                    % Doubled because we are incrementing by 1200 pt samples, not 2400
                    % (buffer is ignored)
                   samples_chirps(sc_counter,:) = samples(a,(b-1)*samples_points+1:b*samples_points);
